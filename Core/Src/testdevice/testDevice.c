...
//			fsSayac = 0;
	...
	if(mode == sutMode){
		...
		if(recievedSize == sensorDataSize) {
			...
			accX = KalmanUpdate(&accXFilter, accX);
			...
			accY = KalmanUpdate(&accYFilter, accY);
			...
			accZ = KalmanUpdate(&accZFilter, accZ);
			...
			pitch = KalmanUpdate(&pitchFilter, pitch);
			...
			roll = KalmanUpdate(&rollFilter, roll);
			...
			yaw = KalmanUpdate(&yawFilter, yaw);
